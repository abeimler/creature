...
class CreatureBattlerSystemApplication : public gamesystem::Application {
    ...
    CreatureBattlerSystemApplication() {
        ...
        this->creatureBattlerSystem = std::make_shared<gamesystem::CreatureBattlerSystem>(this->datamanager);
        
        this->addSystem(this->creatureBattlerSystem);
    }
        
    void init_Entity_withBattlerStatusHalfHitRateFactor(gameentity::Entity entity){
        auto creature_battlerstatuses = entity.component<gamecomp::BattlerStatusesComponent>();
        
        creature_battlerstatuses->statuses_name.push_back(this->halfHitRateFactor_.getName());
    }
...
SCENARIO("Creature Entity with BattlerStatus and update HitRate to Half") {
    GIVEN("Creature Entity") {
        ...
        WHEN("update manager") {
            ...
            THEN("hitrate is set") {
                CHECK(creature_battler->hitrate == CreatureTestData::HITRATE_HALF);
            }
        }
    }
}
