...
void CreatureBattlerCreatureAddExpListener::addExp(gamecomp::CreatureDataComponent& creature_data, 
    ...
    auto nextexp = earr::enum_array_at(creature_battler.attr, +data::Attribute::Exp);
    while (creature_battler.lvl <= maxlvl &&
            creature_battler.exp >= nextexp) {
        creature_battler.lvl++;

        const auto& exp_attrparams = earr::enum_array_at(creature_battler.attrparam, +data::Attribute::Exp);
        ...
        if (creature_battler.lvl >= 0) {
            ...
            if (lvl_index < exp_attrparams_size) {
                earr::enum_array_at(creature_battler.attrbase, +data::Attribute::Exp) = exp_attrparams.at(lvl_index);
            }
        }
...
void CreatureBattlerCreatureAddExpListener::update(const gameevent::CreatureAddExpEvent& event, 
