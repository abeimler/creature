...
void CreatureBattlerCreatureAddExpSystem::update(const gameevent::CreatureAddExpEvent& event, EntityManager& entities, EventBus& events, TimeDelta dt) {
    ...
    for (auto entity : entities.entities_with_components(creature_data, creature_battler)) {
        ...
        while (creature_battler->lvl <= maxlvl && creature_battler->exp >= nextexp) {
            ...
            const auto& exp_attrparams = earr::enum_array_at(creature_battler->attrparam, 
                                                             +data::Attribute::Exp);
            ...
            if (creature_battler->lvl >= 0) {
                ...
                if(lvl_index < exp_attrparams_size) {
                    earr::enum_array_at(creature_battler->attrbase, +data::Attribute::Exp) =
