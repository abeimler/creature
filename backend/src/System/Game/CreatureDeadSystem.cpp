...
void CreatureDeadSystem::makeCreatureDead(EventBus& events, Entity entity,
...
void CreatureDeadSystem::update(
    const gameevent::CreatureMakeDeadEvent& event,
    ...
    for (auto entity : entities.entities_with_components(timers, life, creature_battler)) {
        makeCreatureDead(events, entity, *timers.get(), *life.get(), *creature_battler.get(), event.causeofdeath);
    }
}
