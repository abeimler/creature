...
void CreatureBattlerSystem::updateCreatureBattlerAttribute(
    ...
    creature_battler->lvl = clamp(creature_battler->lvl, 
        creature.getMinLvL(),
        creature.getMaxLvL());
    ...
    creature_battler->hp = clamp(creature_battler->hp, 
        0,
        earr::enum_array_at(creature_battler->attr, +data::Attribute::MaxHP));

    creature_battler->mp = clamp(creature_battler->mp, 
        0,
...
void CreatureBattlerSystem::updateCreatureHitRate(
    ...
    auto sum_battlerstatuses_hitrate_func = [&](const int& init, const std::string& status_name) {
        auto status = this->datamanager_.findCreatureBattlerStatus(status_name);
        return init * ((status)? status->getHitRate() : 1.0);
    };
    
    int status_sum_hitrate = std::accumulate(
        std::begin(battlerstatuses), 
        std::end(battlerstatuses), 
        0, 
...
void CreatureBattlerSystem::updateCreatureCriticalHitRate(
    ...
    auto sum_battlerstatuses_critical_hitrate_func = [&](const int& init, const std::string& status_name) {
        auto status = this->datamanager_.findCreatureBattlerStatus(status_name);
        return init * ((status)? status->getCriticalHitRate() : 1.0);
    };
    
    int status_sum_critical_hitrate = std::accumulate(
        std::begin(battlerstatuses), 
        std::end(battlerstatuses), 
        0, 
...
void CreatureBattlerSystem::updateCreatureEvaRate(
    ...
    auto sum_battlerstatuses_evarate_func = [&](const int& init, const std::string& status_name) {
        auto status = this->datamanager_.findCreatureBattlerStatus(status_name);
        return init * ((status)? status->getEvaRate() : 1.0);
    };
    
    int status_sum_evarate = std::accumulate(
        std::begin(battlerstatuses), 
        std::end(battlerstatuses), 
        0, 
...
void CreatureBattlerSystem::updateCreatureStatusRestriction(
    ...
    auto restriction_comp_func = [&](const std::string& a, const std::string& b) {
        auto status_a = this->datamanager_.findCreatureBattlerStatus(a);
        auto status_b = this->datamanager_.findCreatureBattlerStatus(b);

        data::StatusRestrictionOption status_a_restriction = (status_a)? status_a->getRestriction() : +data::StatusRestrictionOption::None;
        data::StatusRestrictionOption status_b_restriction = (status_b)? status_b->getRestriction() : +data::StatusRestrictionOption::None;

        return status_a_restriction < status_b_restriction;
    };

    auto status_it = std::max_element(std::begin(battlerstatuses),
                                      std::end(battlerstatuses), 
