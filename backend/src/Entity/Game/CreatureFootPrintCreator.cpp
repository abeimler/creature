...
BlockiesAsciiCreator::createBlockyArray(std::string seedstr, size_t size,
                                        size_t scale) {
    auto width = size;
    auto height = size;
    ...
    for (size_t y = 0; y < height; y++) {
        ...
        for (size_t x = 0; x < dataWidth; x++) {
            auto s =
                (util::rand(gen) >= 0.5) ? PIXEL_CHAR_VALUE : PIXEL_CHAR_SPACE;
...
std::string BlockiesAsciiCreator::createBlockyString(std::string seedstr,
                                                     size_t size,
                                                     size_t scale) {
...
std::vector<std::string> CreatureFootPrintCreator::createPrintArray(
    std::string creature_name, data::CreatureLevel creature_level,
...
std::string CreatureFootPrintCreator::createPrintString(
    std::string creature_name, data::CreatureLevel creature_level,
    data::bodymass_t mass, data::bodysize_t bodysize) {
