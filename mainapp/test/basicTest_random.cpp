...
TEST_CASE("random value of range 0-10 with seed 5468792135") {
    ...
    int64_t seednumber = 7; // gen number from seed
...
TEST_CASE("0:1 loses") {
    REQUIRE_FALSE(util::randomOdds(0, 1));
}

TEST_CASE("1:0 wins") { 
    REQUIRE(util::randomOdds(1, 0)); 
}



TEST_CASE("more then 100% wins") {
    REQUIRE(util::randomRate(101));
}

TEST_CASE("100% wins") { 
    REQUIRE(util::randomRate(100)); 
}

TEST_CASE("0% loses") { 
    REQUIRE_FALSE(util::randomRate(0)); 
}
